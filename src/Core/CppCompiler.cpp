...
std::unique_ptr<llvm::Module> CppCompiler::invoke(const std::vector<std::string_view>& sources, llvm::LLVMContext& context)
{
    ...
    clang::CompilerInvocation::CreateFromArgs(compilerInvocation, std::vector<const char*>(), diagnosticsEngine);
    ...
    for (auto it = sources.begin(); it != sources.end(); ++it)
        frontEndOptions.Inputs.push_back(clang::FrontendInputFile(llvm::MemoryBufferRef(it->data(), it->data()), clang::InputKind(clang::Language::CXX)));
